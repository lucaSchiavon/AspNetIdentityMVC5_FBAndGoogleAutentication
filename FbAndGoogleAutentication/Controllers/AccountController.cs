...
namespace FbAndGoogleAutentication.Controllers
{
    [Authorize]
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    //video dove spiega bene come configurare l'email
                    //https://www.youtube.com/watch?v=Rq06iC45Bj0

                    //conferma email serve per evitare di commettere errori nell'imputazione della 
                    //mail nel form di registrazione e per procedere al recupero password. Offre protezione anche dai bot. In genere si desidera impedire 
                    //ai nuovi utenti di inviare dati al sito web prima che siano stati inviati tramite posta elettronica.
                    ...
                    await UserManager.SendEmailAsync(user.Id, "Conferma account", callbackUrl );
                    //+++++++++++++++++++
        ...
        {
            //questo metodo viene chiamato dal link della email di confirm email
            //qui viene confrontato il token assegnato all'utente inviato con l'email con l'id dell'utente (non inviato) e se
            //il token corrisponde a quello dell'utente ...
        ...
        {
            //questo metodo viene chiamato quando si inserisce la mail nel form e si rischiede di resettare la password
            //poichè si è dimenticata
            //il metodo invia una mail con il link dir reset alla casella di posta elettronica inserita nel form
            //e poi redireziona ad una pagina dove viene notificato di controllare nella propria posta elettronica per il reset della pwd
            //il link incluso nell'email chiama la action restpassword
            if (ModelState.IsValid)
            {
            ...
            // Per altre informazioni su come abilitare la conferma dell'account e la reimpostazione della password, vedere https://go.microsoft.com/fwlink/?LinkID=320771
            //questo è un ottimo video per impostare il reset password:
        ...
        {
            //reimposta la password e dirotta sulla pagina di conferma cje il reset password è avvenuto
            if (!ModelState.IsValid)
            {
                return View(model);
            }
